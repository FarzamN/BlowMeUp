...
export const Edit_profile = (
  ...
  return async dispatch => {
    ...
    try {
      ...
      myData.append('instagram', data.instagram);
      myData.append('facebook', data.facebook);
      myData.append('twitter', data.twitter);
      myData.append('youtube', data.youtube);
...
export const create_Video = async (
  ...
  console.log('first', data, type, saveVideo, saveImage)
  try {
    setLoad(true);
...
export const show_own_video = async (setVideoData, loading) => {
  const Data = await AsyncStorage.getItem('user_details');
  const userData = JSON.parse(Data);
  try {
    loading(true)
...
export const likedPost = (data, userData, posts, index) => {
  return async (dispatch) => {
    try {
      const Data = await AsyncStorage.getItem('token');
      const Token = JSON.parse(Data);
      ...
      const responseData = await response.json();
      if (responseData.success.status === 200) {
        posts[index] = responseData.success.data
...
export const Delete_Image = async (data, setDelete, setLoad,navigation) => {
  console.log('data', data);
  setLoad(true);
  try {
...
export const show_all_image = (userDetails) => {
  return async dispatch => {
    try {
      ...
      if (responseData.success.status === 200) {
        dispatch({ type: IMAGE, payload: responseData.success.data.reverse() });
      } else {
        console.log('else error in show_video');
      }
    } catch (error) {
...
export const show_own_image = async (setImageData, load) => {
  const Data = await AsyncStorage.getItem('user_details');
  const userDetails = JSON.parse(Data);
  // return async dispatch => {
  try {
...
export const show_own_streams = async (setStreamData, load) => {
  const Data = await AsyncStorage.getItem('user_details');
  const userDetails = JSON.parse(Data);
  // return async dispatch => {
  try {
...
export const show_all_MucisVideo = async setGetPodcast => {
  try {
    ...
    console.log('catch error in show_all_MucisVideo', error);
  }
};
export const show_all_LiveStream = async (setLiveStream, setLoad) => {
  // return async dispatch => {
  try {
    setLoad(true)
    ...
    setLoad(false)
    console.log('catch error in show_all_LiveStream', error);
  }
};
...
export const Notification = async (setNotification, setLoad) => {
  setLoad(true)
  const Data = await AsyncStorage.getItem('user_details');
  const userDetails = JSON.parse(Data);
  try {
...
export const show_own_song = async (setSongData, load) => {
  const Data = await AsyncStorage.getItem('user_details');
  const userData = JSON.parse(Data);
  try {
    load(true)
...
export const show_search = (userData, setLoading) => {
  return async dispatch => {
    setLoading(true)
    try {
...
export const upload_VR = async (data, setDone, setLoad, setSelect) => {
  console.log('data', data)
  setLoad(true);
  const Data = await AsyncStorage.getItem('user_details');
  const userDetails = JSON.parse(Data);
  ...
  try {
    // let base_url = `${BaseUrl}store-vr/${userDetails.id}`;
    let url = `https://sassolution.org/BlowMeUp/api/store-vr/${userDetails.id}`;
    let myData = new FormData();

    myData.append('link', data.link);
    ...
    const responseData = await response.json();
    if (responseData.success.status == 200) {
      Toast.show('Link has been uploaded');
      setLoad(false);
      setSelect(2);
    } else {
      Toast.show(responseData.error.message);
    ...
    console.log('catch error in upload_VR', error);
    setLoad(false);
  }
};

export const show_own_link = async (setVrData, load) => {
  const Data = await AsyncStorage.getItem('user_details');
  const userData = JSON.parse(Data);
  try {
    load(true)
    let base_url = `https://sassolution.org/BlowMeUp/api/show-vr-id/${userData.id}`;
    ...
    const responseData = await response.json();
    if (responseData.success.status === 200) {
      setVrData(responseData.success.data);
      load(false)
    } else {
      Toast.show('something went wrong');
      load(false)
    ...
    console.log('catch error in show_own_link', error);
    load(false)
  }
};

export const Edit_VrLink = async (
  data,
  navigation,
  Values
) => {
  try {
    let base_url = `${BaseUrl}edit-vr/${Values.id}`;
    let myData = new FormData();
    ...
    if (responseData?.success?.status == 200) {
      navigation.goBack();
      Toast.show('Your Link is updated');
    } else {
      Toast.show('something went wrong');
    }
  } catch (error) {
...
export const Delete_VrLink = async (value, setDelete, setLoad) => {
  console.log('value', value)
  setLoad(true);
  try {
    let base_url = `${BaseUrl}delete-vr/${value.id}`;
...
export const show_all_VrLinks = async (setLinks, load) => {
  try {
    load(true)
    let base_url = `${BaseUrl}show-vr`;
    ...
    if (responseData.success.status === 200) {
      setLinks(responseData.success.data.reverse());
      load(false)
    } else {
      load(false)
      Toast.show('something went wrong');
    ...
    load(false)
    console.log('catch error in show_all_VrLinks', error);
  }
};