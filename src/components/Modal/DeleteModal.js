...
const DeleteModal = ({visible,KeepPress,DeletePress}) => {
  return (
    <Modal isVisible={visible} style={GlobalStyle.MainModal} >
      <View style={[GlobalStyle.ModalContainer, styles.ModalContainer]}>
        <View style={styles.iconBox}>
          <Ionicons name="warning" color={Colors.Danger} size={scale(30)} />
        </View>
        <Text style={styles.Heading}>Delete Video</Text>
        <Text style={styles.SubHeading}>
          {`You're`} going to delete the Video. Are you sure?
        </Text>
        <View style={[GlobalStyle.Row, {justifyContent: 'space-evenly'}]}>
          <CustomButton
            onPress={KeepPress}
          textStyle={styles.textStyle}
            containerStyle={[styles.containerStyle,{backgroundColor:'#F5F5F7'}]}
            title="No, Keep It."
          />
          <CustomButton
          onPress={DeletePress}
